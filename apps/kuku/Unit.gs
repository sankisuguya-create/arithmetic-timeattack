...
var UNIT = {
  ...
  tips:
    '<b>weak_child</b>：児童ごと・段ごとの平均想起時間(秒)。<br>' +
    '色は「学年のまん中の何倍か」。積が大きい段ほど誰でも遅い（問題サイズ効果）ので、<br>' +
    '秒で線を引くと7・8の段が全員赤くなり、個人差が見えなくなる。<br>' +
    '九九は既習なので正答率では差が出ない。差が出るのは想起の速さ。<br><br>' +
    '・同じ段がかけ算・わり算の両方で赤 → 想起そのものが未定着<br>' +
    '・わり算だけ赤 → 逆算への変換が課題<br>' +
    '・「── ○年のまん中 ──」の行が赤 → その段は学年ぜんたいが遅い。個別でなく授業の的<br>' +
    '・取りこぼしが大きい段 → その段の平均は「答えられた問題だけの平均」で、実力より速い<br>' +
