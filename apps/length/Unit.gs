...
var UNIT = {
   ...
   * G・Hはめもりの読み取り範囲から桁数が一意に決まるので、ui.html 側の
   * ruler 用デフォルトに頼らずここで明示する（他の型はA〜Fのように
   * gen側の値域設計で桁数を揃えているため未宣言＝既定動作のままでよい）。
   */
  digitCap: {
    5: { cm: 2 },   // G：10〜78cmの範囲内なので常に2桁
    6: { cm: 3 }    // H：cm欄だけで「187」のように答える書き方も認めるため3桁固定
  },
